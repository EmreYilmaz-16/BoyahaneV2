...
<cftry>
    ...
    <cfset username = structKeyExists(payload, "username") ? trim(payload.username) : "">
    <cfif NOT len(username)>
        <cfheader statuscode="400" statustext="Bad Request"><cfoutput>#serializeJSON({"success"=false,"message"="username gerekli."})#</cfoutput><cfabort>
    </cfif>
    ...
    <cfquery name="getUser" datasource="boyahane">
        ...
        FROM kullanicilar
        WHERE password = <cfqueryparam value="#password#" cfsqltype="cf_sql_varchar"> and username =  <cfqueryparam value="#username#" cfsqltype="cf_sql_varchar">
          AND is_active = true
