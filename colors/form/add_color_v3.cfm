...
function AgacaEkle(stock_id) {
    $.ajax({
        url    : 'cfc/boyahane.cfc?method=getProdTreeWithName_yenia&stock_id=' + stock_id,
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < tree.length; j++) {
            ...
            recipeData.push({
                ...
                tree_id                : t.TREE_ID || 0,
                related_tree_id        : t.RELATED_TREE_ID || 0
            });
        }
    }
...
<script src="/colors/form/agac_funkv3.js"></script>
</cfoutput>
