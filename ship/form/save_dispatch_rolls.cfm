...
<cftry>
    ...
    <cfif len(mobileUserId)>
        ...
        <cfif NOT checkMobileUser.recordCount>
            <cfoutput>#serializeJSON({"success"=false,"message"="Yetkisiz erişim. poza"})#</cfoutput><cfabort>
        </cfif>
    <cfelseif NOT (structKeyExists(session, "authenticated") AND session.authenticated)>
        <cfoutput>#serializeJSON({"success"=false,"message"="Yetkisiz erişim. pozb"})#</cfoutput><cfabort>
    </cfif>
