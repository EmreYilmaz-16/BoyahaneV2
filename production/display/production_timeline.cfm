...
<div class="timeline-page">
  ...
  <main class="tl-main">
    ...
    <div class="tl-tools">
      ...
      <span style="color:##64748b;font-size:.82rem">Saat hassasiyeti: 15 dakika · Güncel saat kırmızı çizgiyle gösterilir · Sağ tuş: plandan kaldır</span>
    </div>
    <div class="tl-scroll" id="timelineScroll"><div class="tl-ruler" id="tlRuler"></div><div id="lanes"></div></div>
  </main>
</div>

<div class="tl-context-menu" id="jobContextMenu"><button type="button" onclick="unplanFromContext()">Plandan kaldır / iptal et</button></div>
...
var START=new Date('#dateFormat(timelineStart,"yyyy-mm-dd")#T00:00:00'), DAYS=#viewDays#, PX_PER_DAY=576, MINUTES_PER_PX=1440/PX_PER_DAY, DROP_MODE='insert', DRAGGED_ORDER_ID=null, CONTEXT_ORDER_ID=null;
...
function renderTimeline(){var ruler=document.getElementById('tlRuler'), lanes=document.getElementById('lanes');ruler.innerHTML='';lanes.innerHTML='';for(var i=0;i<DAYS;i++){var d=new Date(START.getTime()+i*86400000), cell=document.createElement('div'), hours=[];cell.className='tl-day';for(var h=0;h<24;h++){hours.push('<span class="tl-hour">'+pad(h)+':00</span>');}cell.innerHTML='<div class="tl-day-title">'+pad(d.getDate())+'.'+pad(d.getMonth()+1)+'.'+d.getFullYear()+'</div><div class="tl-hours">'+hours.join('')+'</div>';ruler.appendChild(cell);}STATIONS.forEach(function(st){var lane=document.createElement('div');lane.className='lane';lane.innerHTML='<div class="lane-label">'+enc(st.text)+'</div><div class="lane-track" data-station="'+st.id+'" style="width:'+(DAYS*PX_PER_DAY)+'px"></div>';var track=lane.querySelector('.lane-track');track.ondragover=function(ev){ev.preventDefault();track.classList.add('drop-hint');};track.ondragleave=function(){track.classList.remove('drop-hint');};track.ondrop=function(ev){ev.preventDefault();track.classList.remove('drop-hint');var id=getDragOrderId(ev), r=track.getBoundingClientRect(), x=ev.clientX-r.left+track.scrollLeft;if(id) planOrder(id,st.id,snapDateFromX(x),DROP_MODE);};PLANNED.filter(function(j){return j.station_id==st.id;}).forEach(function(j){var job=document.createElement('div');job.className='job '+(j.is_urgent?'urgent':'');job.style.left=pxFromDate(j.startDate)+'px';job.style.width=widthFromDates(j.startDate,j.endDate)+'px';job.title='Sürükle: makine/saat değiştir · Sağ tuş: plandan kaldır';job.draggable=true;job.dataset.id=j.p_order_id;job.ondragstart=function(ev){DRAGGED_ORDER_ID=j.p_order_id;setDragData(ev,j.p_order_id,'planned');job.classList.add('dragging');};job.ondragend=function(){DRAGGED_ORDER_ID=null;job.classList.remove('dragging');};job.ondragover=function(ev){if(DROP_MODE==='swap'&&String(DRAGGED_ORDER_ID)!==String(j.p_order_id)){ev.preventDefault();job.classList.add('swap-target');}};job.ondragleave=function(){job.classList.remove('swap-target');};job.ondrop=function(ev){if(DROP_MODE!=='swap')return;ev.preventDefault();ev.stopPropagation();job.classList.remove('swap-target');var sourceId=getDragOrderId(ev);if(sourceId&&String(sourceId)!==String(j.p_order_id)) swapOrders(sourceId,j.p_order_id);};job.onclick=function(){hideJobContextMenu();openOrderModal(j);};job.oncontextmenu=function(ev){ev.preventDefault();ev.stopPropagation();showJobContextMenu(ev,j.p_order_id);};job.innerHTML='<b>'+enc(j.p_order_no)+'</b><small>'+enc(j.color_code)+' '+enc(j.color_name)+'</small><small><span class="job-chip">'+serverDate(new Date(j.startDate)).slice(11,16)+'</span> <span class="job-chip">'+serverDate(new Date(j.endDate)).slice(11,16)+'</span></small>';track.appendChild(job);});addNowLine(track);lanes.appendChild(lane);});}
...
function addNowLine(track){var now=new Date(), viewEnd=new Date(START.getTime()+DAYS*86400000);if(now<START||now>viewEnd)return;var line=document.createElement('div');line.className='now-line';line.style.left=pxFromDate(now)+'px';track.appendChild(line);}
function showJobContextMenu(ev,id){CONTEXT_ORDER_ID=id;var menu=document.getElementById('jobContextMenu');menu.style.left=ev.clientX+'px';menu.style.top=ev.clientY+'px';menu.classList.add('open');}
function hideJobContextMenu(){var menu=document.getElementById('jobContextMenu');if(menu)menu.classList.remove('open');}
function unplanFromContext(){var id=CONTEXT_ORDER_ID;hideJobContextMenu();if(id)unplan(id);}
document.addEventListener('click',hideJobContextMenu);document.addEventListener('keydown',function(ev){if(ev.key==='Escape')hideJobContextMenu();});document.addEventListener('scroll',hideJobContextMenu,true);
