...
function itemClass(o){var cls='production-job';if(Number(o.status)===5)cls+=' completed';else if(Number(o.active_pause_count||0)>0)cls+=' paused';if(o.is_urgent)cls+=' urgent';return cls;}
...
function mergePlannedOrders(data){PLANNED=data||[];var nextIds={};PLANNED.forEach(function(o){nextIds[o.p_order_id]=true;items.update({id:o.p_order_id,group:o.station_id,start:o.startDate,end:o.endDate,content:itemContent(o),className:itemClass(o),order:o});});items.getIds().forEach(function(id){if(!nextIds[id])items.remove(id);});}
