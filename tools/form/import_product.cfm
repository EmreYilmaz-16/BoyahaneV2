...
<div class="page-header">
    <div class="page-header-left">
        ...
        <div class="page-header-title">
            <h1>Excel'den Ürün İçe Aktar</h1>
            <p>Excel dosyasından toplu ürün aktarımı yapın</p>
        </div>
    </div>
            ...
            <div class="collapse show" id="collapseInstructions">
                <div class="card-body p-3">
                    <ol class="mb-2">
                        ...
                        <li>Şablona ürünlerinizi doldurun. <strong>Ürün Adı</strong> ve <strong>Kategori ID</strong> zorunludur.</li>
                        <li>Kategori ID için aşağıdaki referans tablosuna bakabilirsiniz. Kategori adını da yazabilirsiniz.</li>
                    ...
                    <div class="alert alert-warning py-2 mb-0">
                        ...
                        <strong>Zorunlu alanlar:</strong> Ürün Adı, Kategori ID &nbsp;|&nbsp;
                        ...
                        <div class="card-body p-0" style="max-height:200px;overflow-y:auto;">
                            <table class="table table-sm table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><strong>#product_catid#</strong></td>
                                        <td><small>#hierarchy# - #product_cat#</small></td>
                                    </tr>
                                    </cfoutput>
                                </tbody>
                            </table>
...
const FIELDS = [
    ...
    { key: 'product_catid',   label: 'Kategori',        required: true  },
...
const AUTO_HEADER_MAP = {
    ...
    'kategori id':'product_catid', 'kategori':'product_catid', 'kategorı':'product_catid',
    'product_catid':'product_catid', 'cat id':'product_catid', 'category id':'product_catid',
    'category':'product_catid', 'kategori adı':'product_catid',
...
/* ====================================================
   ...
   ==================================================== */
let excelHeaders = [];
let excelRows    = [];  // ham string satırlar
let columnMapping = {}; // excelCol -> fieldKey | null
...
function resolveCategoryId(val) {
    ...
    const cat = CATEGORIES.find(c => c.name.toLowerCase() === name || c.display.toLowerCase() === name);
    return cat ? cat.id : 0;
}
...
function buildPreview() {
    // Zorunlu alan kontrolü - en az product_name ve product_catid eşleştirilmiş olmalı
    const hasProdName = Object.values(columnMapping).includes('product_name');
    const hasCatId    = Object.values(columnMapping).includes('product_catid');
    if (!hasProdName) { alert('"Ürün Adı" alanını bir sütuna eşleştirmeniz zorunludur.'); return; }
    if (!hasCatId)    { alert('"Kategori" alanını bir sütuna eşleştirmeniz zorunludur.'); return; }
    ...
    for (let i = 0; i < maxPreview; i++) {
        ...
        const catRaw = raw[fieldToColIdx['product_catid']] !== undefined ? raw[fieldToColIdx['product_catid']] : '';
        const catId  = resolveCategoryId(catRaw);

        if (!pName)  errors.push('Ürün adı boş');
        if (!catId)  errors.push('Geçersiz kategori: "' + catRaw + '"');
        ...
        const cells = mappedFields.map(f => {
            ...
            if (f.key === 'product_catid') val = catId ? getCategoryDisplay(catId) : '⚠ ' + catRaw;
            else if (f.key === 'brand_id') { const bid = resolveBrandId(val); val = bid ? getBrandDisplay(bid) : (val ? '⚠ ' + val : ''); }
            else if (f.key === 'product_status' || f.key === 'is_sales' || f.key === 'is_purchase')
                val = toBool(val, true) ? '✓ Evet' : '✗ Hayır';
            else if (f.key === 'tax') val = (parseFloat(val) || 18) + '%';
        ...
        rows.push('<tr class="' + rowClass + '"><td>' + (i + 2) + '</td>' + cells + statusCell + '</tr>');
    }
    ...
    if (validCount === 0) {
        alert('Geçerli kayıt bulunamadı. Lütfen verileri ve sütun eşleştirmelerini kontrol edin.');
        return;
    }
